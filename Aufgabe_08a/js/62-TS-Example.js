...
let monsterHolder = "monsterHoldingCell"; // ID fÃ¼r das Haupt-Element, in welchem die Monster sich befinden werden. Wird vielleicht mehrfach in dem Skript gebraucht, deshalb einmalig definitiert.
...
let playerXPperLevel = 500; // Da es nur einen Spieler gibt, ergibt sich noch nicht viel Sinn darin, fÃ¼r den Spieler ein interface (im Sinne der Programmierung) zu erstellen.
...
let monsterImage = ["p1.png", "g5.gif", "g3.gif", "g7.gif"];
let ArrayPush = [];
...
function generateMonster() {
    let newMonsterName = generateMonsterName(); // Eigens-gebaute Funktion, welche einen string zurÃ¼ck gibt.
    let newMonsterHP = generateMonsterHitPoints(); // Eigens-gebaute Funktion, welche eine Zahl zurÃ¼ck gibt.
    let newMonsterXP = generateMonsterXP(); // Eigens-gebaute Funktion, welche eine Zahl zurÃ¼ck gibt.
    let newMonsterModifier = generateMonsterModifer(); // Eigens-gebaute Funktion, welche ein string-Array zurÃ¼ck gibt.
    let newMonsterAge = generateMonsterAge(); //Funktion fÃ¼r Alter des Monster
    let newMonsterImage = generateMonsterImage(); //Funktion fÃ¼r Monster-Image
    let newMonster = {
...
function getRNGNumber(_maxNumber) {
    let rngNumber = Math.random(); // Macht folgendes: Generiere eine zufÃ¤llige Komma-Zahl zwischen 0 - 1.
...
function generateMonsterName() {
    let generatedMonsterName = ""; // Erstelle einen leeren String fÃ¼r das Monster
    // Monster-Vorname
    // Mathematik! Hier wird eine zufÃ¤llig-generierte Zahl benÃ¶tigt.
    let rngNumber = getRNGNumber(prefix.length); // Der RÃ¼ckgabewert der Funktion wird hier verwendet um den entsprechenden Teil des Namens (hier: Anfang) zu generieren.
